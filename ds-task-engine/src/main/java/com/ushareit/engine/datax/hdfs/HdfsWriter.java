...
public class HdfsWriter extends Writer {
    ...
    public HdfsWriter(Context context){
        ...
        if(context.getRuntimeConfig().getCatalog().getTables().get(0).getAutoCreateTable()){
            ...
            String targetTable = context.getRuntimeConfig().getCatalog().getTables().get(0).getTargetTable();
            ...
            if(StringUtils.isNotBlank(table.getPartitions())){
                if(location.endsWith("/")){
                    location += table.getPartitions().replaceAll(",","/");
                }else{
                    location += "/"+table.getPartitions().replaceAll(",","/");
                }
            }
