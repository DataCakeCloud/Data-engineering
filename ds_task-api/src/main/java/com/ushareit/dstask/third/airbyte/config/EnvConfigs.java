...
public class EnvConfigs implements Configs {
    ...
    public static final Map<String, Function<EnvConfigs, String>> JOB_SHARED_ENVS = ImmutableMap.of(
    ...
    public WorkerEnvironment getWorkerEnvironment() {
        return getEnvOrDefault(WORKER_ENVIRONMENT, WorkerEnvironment.KUBERNETES, s -> WorkerEnvironment.valueOf(s.toUpperCase()));
    }
    ...
    public SecretPersistenceType getSecretPersistenceType() {
        final String secretPersistenceStr = getEnvOrDefault(SECRET_PERSISTENCE, SecretPersistenceType.TESTING_CONFIG_DB_TABLE.name());
    ...
    public Map<String, String> getJobDefaultEnvMap() {
        ...
        final Map<String, String> jobSharedEnvMap = JOB_SHARED_ENVS.entrySet().stream().collect(Collectors.toMap(
                ...
                entry -> Exceptions.swallowWithDefault(() -> {
                    if (entry.getValue().apply(this) != null) {
