...
import com.ushareit.dstask.third.airbyte.utils.MoreMappers;
import com.ushareit.dstask.third.airbyte.utils.MoreStreams;
...
public class Jsons {
    ...
    public static <T> T deserialize(final String jsonString, final TypeReference<T> typeReference) {
        try {
    ...
    public static Set<String> keys(final JsonNode jsonNode) {
        if (jsonNode.isObject()) {
            return Jsons.object(jsonNode, new TypeReference<Map<String, Object>>() {
            }).keySet();
        } else {
            return new HashSet<>();
        }
