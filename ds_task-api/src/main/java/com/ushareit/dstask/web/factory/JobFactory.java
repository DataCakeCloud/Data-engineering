...
public class JobFactory {
    ...
    public static Job getJob(Task task, Integer tagId, Integer savepointId, TaskServiceImpl taskServiceImpl) {
        ...
        if("local".equals(runtimeConfigObject.getString("executeMode"))){
            localMode = true;
        }
        ...
        String sparkMode = taskServiceImpl.getSparkMode();
        switch (TemplateEnum.valueOf(task.getTemplateCode())) {
            ...
            case Mysql2Hive:
                if (localMode) return new LocalTaskJob(task,taskServiceImpl);
                if (syncType == null) {
            ...
            case Db2Db:
                return new LocalTaskJob(task,taskServiceImpl);
            case Oracle2Hive:
            case SqlServerHive:
            case Doris2Hive:
            ...
            case Hive2Hive:
                ...
                if("local".equalsIgnoreCase(sparkMode) && engine!=null && engine.contains("spark")){
