 ...
 */
@Slf4j
public class SinkIcebergJob extends DataLakeJob {
    ...
    public static SinkIcebergJob sinkIcebergJob;
    ...
    public SinkIcebergJob(Task task, Integer tagId, Integer savepointId, TaskServiceImpl taskServiceImp) {
    ...
    public String getCatalogStr(String region) {
        String cataLog = "CREATE CATALOG %s WITH (\n" +
                "  'type'='iceberg',\n" +
                "  'catalog-type'='hive',\n" +
                "  'uri'='%s',\n" +
                "  'clients'='5',\n" +
        ...
        return String.format(cataLog, catlogNmae, hmsUri, cloudResource.getStorage() + "flink/datacake/");
    }

    public Object getSelectColumns(List<Column> columns) {
        StringBuffer sb = getBaseSelectColumns(columns);
        sb.append(DATEPART_SQL).append("\n");
        sb.append(HOUR_SQL);
        return sb.toString();
    }
    ...
    protected StringBuffer getBaseSelectColumns(List<Column> columns) {
        ...
        columns.stream().forEach(metadata -> sb.append(" `" + metadata.getName() + "`, \n"));
        return sb;
    }
