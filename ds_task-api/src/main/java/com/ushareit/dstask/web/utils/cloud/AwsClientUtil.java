...
public class AwsClientUtil extends CloudBaseClientUtil {
    ...
    private static Map<String, AmazonS3> s3ClientMap = new HashMap<>();
    ...
    public String upload(MultipartFile multipartFile, String subPath, String region) {
        try {
            ...
            AmazonS3 client = getClient(realRegion);
            //不需要检测路径 桶存在即可
...
//        String region = getRegion(bucketName);
        CloudResouce.DataResource cloudResource = cloudResourcesService.getCloudResource(region);
        ListObjectsV2Result result = getClient(cloudResource.getRegion()).listObjectsV2(bucketName, path);
...
//        String objName = matcher.group(5) + "/" + matcher.group(6);
        ...
        try {
            CloudResouce.DataResource cloudResource = cloudResourcesService.getCloudResource(region);
            getClient(cloudResource.getRegion()).deleteObject(bucketName, objName);
    ...
    public static AmazonS3 getClient(String region) {
        AmazonS3 s3 = s3ClientMap.get(region);
        if (s3 != null) {
            return s3;
        }
