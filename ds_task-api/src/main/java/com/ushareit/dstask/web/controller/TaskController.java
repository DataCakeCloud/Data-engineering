...
public class TaskController extends BaseBusinessController<Task> {
    ...
    public BaseResponse renderSql(@RequestBody Map<String, String> requestBody) {
        return BaseResponse.success(taskService.renderSql(requestBody.get("sql"), requestBody.get("executionDate")));
    }
...
//        task.setTenancyCode(groupName.split(",")[0]);
    ...
    @ApiOperation(value = "批量更新告警")
    @PostMapping("/batchupdateAlert")
    public BaseResponse batchupdateAlert(@RequestBody @Valid BatchUpdateAlert batchUpdateAlert) {
        try {
            taskService.batchUpdateAlert(batchUpdateAlert);
            return BaseResponse.success();
        } catch (Exception e) {
            e.printStackTrace();
            return BaseResponse.error(BaseResponseCodeEnum.BATCH_UPDATE_ROLE_FAILED, CommonUtil.printStackTraceToString(e));
