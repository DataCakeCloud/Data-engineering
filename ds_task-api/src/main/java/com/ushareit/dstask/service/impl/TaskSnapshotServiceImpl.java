...
import com.ushareit.dstask.web.utils.HDFSUtils;
...
//        List<TaskSnapshot> checkpoint = saveCheckpoint(taskSnapshot.getTaskId());
    ...
    public ObsS3Object getLatestCheckpoint(Integer taskid){
        ...
        CloudBaseClientUtil cloudClientUtil = cloudFactory.getCloudClientUtilByUrl(flinkCluster.getStatePath());
...
//        List<ObsS3Object> obsS3Objects = cloudClientUtil.listObjects(taskid, taskInstance.getEngineInstanceId(), flinkCluster.getStatePath(), env,flinkCluster.getRegion());
        List<ObsS3Object> obsS3Objects = null;
        String hdfs_address = null;
        String path = null;
        if (flinkCluster.getStatePath().startsWith("hdfs")) {
            try {
                hdfs_address = sysDictService.getValueByCode("HDFS_ADDRESS");
                path = flinkCluster.getStatePath() + File.separator + env + File.separator + taskid +
                        DsTaskConstant.CHECKPOINT_PATH + File.separator + taskInstance.getEngineInstanceId() + File.separator;
                obsS3Objects = HDFSUtils.getHdfsStatus(hdfs_address, path);
            } catch (Exception e) {
                log.info("hdfs hdfs_address is " + hdfs_address + ",path is " + path);
                log.info("hdfs get stats fail");
            }
        } else {
            CloudBaseClientUtil cloudClientUtil = cloudFactory.getCloudClientUtilByUrl(flinkCluster.getStatePath());
            obsS3Objects = cloudClientUtil.listObjects(taskid, taskInstance.getEngineInstanceId(), flinkCluster.getStatePath(), env, flinkCluster.getRegion());
        }
